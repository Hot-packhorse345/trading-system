...
pub fn split_indicator_def(
    ...
    let ind_type = obj.get("type").and_then(|v| v.as_str()).map(|s| s.to_string());
    let timeframe = obj.get("timeframe").and_then(|v| v.as_str()).map(|s| s.to_string());
    let params = obj.iter().filter(|(k, _)| *k != "type" && *k != "timeframe").map(|(k, v)| (k.clone(), v.clone())).collect();
...
impl IndicatorConfig {
    pub fn canonical_json(&self) -> String {
        ...
        let sorted: BTreeMap<&str, &Value> = self.params.iter().map(|(k, v)| (k.as_str(), v)).collect();
    ...
    fn p_usize(&self, k: &str, d: usize) -> usize {
        for key in &[k, "timeperiod", "length", "period"] {
            if let Some(v) = self.params.get(*key).and_then(|v| v.as_u64()) { return v as usize; }
        }
        d
    }
    fn p_period(&self, primary: &str, d: usize) -> usize {
        self.params.get(primary).or_else(|| self.params.get("timeperiod"))
                                .or_else(|| self.params.get("length"))
                                .or_else(|| self.params.get("period"))
                                .and_then(|v| v.as_u64()).map(|v| v as usize).unwrap_or(d)
    }
...
pub fn build(cfg: &IndicatorConfig) -> Result<Box<dyn Indicator>> {
    match cfg.kind.as_str() {
        ...
        "sma" => Ok(Box::new(Sma { period: cfg.p_period("period", 20) })),
        "wma" => Ok(Box::new(Wma { period: cfg.p_period("period", 20) })),
